...
const HTTP_PROXY = 'socks5://127.0.0.1:9050'; // Set to 'socks5://127.0.0.1:9150' for Tor, or leave as '' if not using a proxy.
...
// Array to store all stream IDs during the run
let allStreamUrls = [];

// Function to save all stream IDs to stream-urls.json at the end of the run
...
function collectStreamId(eventUrl, streamId) {
  // Remove BASE_URL from the eventUrl
...
(async () => {
  try {
    ...
    const browser = await firefox.launch({
      headless: true,
      proxy: HTTP_PROXY ? { server: HTTP_PROXY } : undefined, // Use proxy only if it's set
    });
